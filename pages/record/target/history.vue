<template>
    <div>
        <HeaderTab icon="mdi-bullseye-arrow" back="/record" />
        <HeaderTitle icon="record" title="ประวัติเป้าหมาย" />
...
export default {
    ...
    async fetch() {
        const data = await this.$store.dispatch("record/target/history", { date : this.date });
        this.items = data
    },
    methods: {
        changeDate() {
        ...
        async showEdit(id) {
            this.id = id;
            const data = await this.$store.dispatch("record/target/findOne", id);
        ...
        async submit() {
            ...
            await this.$store.dispatch("record/target/update", { id: this.id, data: this.form }).then(response => {
